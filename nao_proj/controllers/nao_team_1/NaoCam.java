...
public class NaoCam {
  ...
  private void findColorBlob(int R, int G, int B, int threshold) {
    ...
    public void findMajorityColorBlob(int R, int G, int B, int threshold) {
    ...
    if (npixels > image.length*0.8) {
      colorIsMajority=true;
    }
    else {
      colorIsMajority=false;
    }
  ...
  public void processImage() {
    ...
    // find orange ball
    findColorBlob(240, 140, 50, 60);//threshold used to be 60
   // if(blobDirectionAngle == UNKNOWN)
   // findColorBlob(185, 115, 40, 60);//threshold used to be 60
  ...
  public boolean getcolorIsMajority() {
    return colorIsMajority;
  }
