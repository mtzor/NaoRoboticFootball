...
public abstract class Player extends Robot {
  ...
  protected Motion backwardsMotion, forwardsMotion, forwards50Motion;
  protected Motion sideStepRightMotion, sideStepLeftMotion,shootMotion;
  ...
  protected DistanceSensor sonarRight, sonarLeft;
  ...
  protected TouchSensor RFBumperRight,LFBumperRight,RFBumperLeft,LFBumperLeft;
  ...
  public Player(int playerID, int teamID) {
    ...
    sonarRight = getDistanceSensor("Sonar/Right");
    sonarRight.enable(SIMULATION_STEP);
    
    sonarLeft = getDistanceSensor("Sonar/Left");
    sonarLeft.enable(SIMULATION_STEP);
    ...
    for (int i = 0; i < TOUCH_SENSOR_NAMES.length; i++) {
      ...
      fsr[i].enable(SIMULATION_STEP);  // uncomment only if needed !LATER USE THEM TO DETECT FALL ???????????
    }
    
    RFBumperRight=getTouchSensor("RFoot/Bumper/Right");
    RFBumperRight.enable(SIMULATION_STEP);
    RFBumperLeft=getTouchSensor("RFoot/Bumper/Left");
    RFBumperLeft.enable(SIMULATION_STEP);
    LFBumperRight=getTouchSensor("LFoot/Bumper/Right");
    LFBumperRight.enable(SIMULATION_STEP);
    LFBumperLeft=getTouchSensor("LFoot/Bumper/Left");
    LFBumperLeft.enable(SIMULATION_STEP);

    // emitter/receiver devices that can be used for inter-robot communication 
    //USE THEM TO SEND MESSAGES TO TEAM
    ...
    // topLeftUltrasound = getDistanceSensor("USSensor3"); //NEEDED TO DETECT OTHER ROBOTS FROM OTHER TEAM
  ...
  boolean bumpedLeft,bumpedRight;
 ...
 int randSteps=rand.nextInt(50);//10
 boolean isCloseRight=false;
  boolean isCloseLeft=false;
  protected void playMotion(Motion motion) {
    ...
    do {
      runStep();
      bumpedLeft=(LFBumperRight.getValue()==1 || LFBumperLeft.getValue()==1);
      bumpedRight=(RFBumperRight.getValue()==1 || RFBumperLeft.getValue()==1);
      isCloseRight=(sonarRight.getValue()<0.4);
      isCloseLeft=(sonarLeft.getValue()<0.4);
      //System.out.println("Bumped Right "+bumpedRight);
     // System.out.println("Bumped Left "+bumpedLeft);
    }
    while (! motion.isOver() && !bumpedLeft && !bumpedRight && !isCloseLeft && !isCloseRight);
   
  if(! motion.isOver()){
    if(!(motion==sideStepRightMotion) && !(motion==sideStepLeftMotion) && !((motion==shootMotion))){
    motion.stop();  
    sleepSteps(randSteps);
    System.out.println(" NOT SIDE MOTION");

    }  
   // System.out.println("STOPPED MOTION");
    
    //CollisionAvoidanceSequence();
  // // if(bumpedLeft || isCloseLeft)
    if(isCloseLeft){
     ...
     System.out.println("Bumped Right ");
     playMotion(sideStepLeftMotion);
     }
     else if (bumpedLeft && motion!=shootMotion){
     System.out.println("Bumped Right ");
  ...
  // use inertial unit to detect fall //NEEDS WORK
  protected void getUpIfNecessary() {
    double[] rpy = inertialUnit.getRollPitchYaw();
    // System.out.println("rpy[0] "+rpy[0]+"rpy[1]"+rpy[1]);
    // System.out.println("Player:"+playerID );
     
    if (Math.abs(rpy[0]) >3.142 || Math.abs(rpy[1]) > 3.142){//used to be 3.142
      playMotion(standUpFromFrontMotion);
      System.out.println("Trying to get up !!!");}

  }

  protected void getUpIfNecessary1() {
    System.out.println("IN GET UP 1 FUNCTION");
  
    //headPitch.setPosition(0.0);  // horizontal head
   // headYaw.setPosition(0.0);
    camera.selectTop();  // use top camera
    
    //check if you see grass
    //preform motion
    //camera.findMajorityColorBlob(15,40,20,60);
    camera.findMajorityColorBlob(45,55,95,60);    
    if (camera.getcolorIsMajority()){
      playMotion(standUpFromFrontMotion);
      System.out.println("Trying to get up 1!!!");}
  ...
  protected void readIncomingMessages() {// NEEDED TO READ TEAMMATES MESSAGES
